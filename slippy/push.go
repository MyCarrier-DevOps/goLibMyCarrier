...
//     no benefit. "Will dispatch nothing" is PushOptions.Dispatch when the caller states a
//     RECOGNIZED value (see DispatchIntent); DispatchIntentUnspecified and any unrecognized
//     value both fall back to len(opts.Components) == 0 — component count is neither
//     necessary nor sufficient on its own, which is why the field exists. That distinction
//     matters to a consumer author: a mis-cased or mis-serialized value crossing the
//     slippy-api JSON boundary is silently ignored rather than honored. A caller that
//     dispatches work without build components (a tests-only repo) MUST set
//     DispatchIntentSomething or this guard suppresses the retrigger it wanted.
//
//     The guard additionally never claims a `failed` slip, regardless of intent — see
//     emptyRunGuardApplies for why that carve-out exists independently of this field.
