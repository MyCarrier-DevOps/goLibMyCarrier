...
// Used for tests that need an env var explicitly *unset*; t.Setenv only covers
// the set case (and restores on cleanup automatically).
...
func TestResolveAPIURL(t *testing.T) {
	cases := []struct {
		...
		{
			name:      "explicit POLLY_API_URL wins over namespace",
		...
		},
		{
			...
			explicit: "http://polly-api.argo-events.svc.cluster.local:8080",
			expSet:   true,
			wantURL:  "http://polly-api.argo-events.svc.cluster.local:8080",
		},
		{
		...
		},
		{
			...
			nsSet:     true,
			wantURL:   "http://polly-api-test.argo-events-test.svc.cluster.local:8080",
		},
		{
		...
		},
		{
			...
			explicit: "polly-api.example.com",
			expSet:   true,
			wantErr:  true,
		},
		{
			name:     "explicit override with non-http scheme returns error",
			explicit: "ftp://polly-api.example.com",
			expSet:   true,
			wantErr:  true,
		},
		{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr {
				if !strings.Contains(err.Error(), envAPIURL) {
					t.Errorf("error should mention %s for operator clarity, got: %v", envAPIURL, err)
				}
				...
				if tc.nsSet && tc.namespace != "" && !tc.expSet {
					if !strings.Contains(err.Error(), tc.namespace) {
						t.Errorf("error should echo offending namespace %q, got: %v", tc.namespace, err)
					}
				}
			}
		})
